...
def _rand_scale(scale):
    ...
    return scale if (np.random.randint(2) == 0) else 1.0 / scale
...
def apply_random_scale_and_crop(image, new_w, new_h, net_w, net_h, dx, dy):
    ...
    if dx > 0:
        im_sized = np.pad(
            im_sized, ((0, 0), (dx, 0), (0, 0)), mode="constant", constant_values=127
        )
    else:
    ...
    if dy > 0:
        im_sized = np.pad(
            im_sized, ((dy, 0), (0, 0), (0, 0)), mode="constant", constant_values=127
        )
    else:
